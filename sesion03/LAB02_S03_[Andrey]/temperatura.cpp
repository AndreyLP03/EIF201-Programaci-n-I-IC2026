...
double* crearRegistro(int& cantidadDias) {
	...
	std::cout << " Ingrese la cantidad de dias a registrar: " << std::endl;
	std::cin >> cantidadDias;

	while (cantidadDias <= 0) {
		std::cout << " Error debe ser mayor a cero , intentelo de nuevo: " << std::endl;
		std::cin >> cantidadDias;
	}
...
void ingresarTemperaturas(double* registro, int cantidad) {
	std::cout << "Ingrese las temperaturas en grados celsios:  "<<std::endl;
